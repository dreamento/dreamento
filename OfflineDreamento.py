...
# =============================================================================
    ...
    #%% Checkbox for plotting spectrograms with markers
        self.plot_tfr_EEG = IntVar(value = 1)
        self.checkbox_plot_tfr_EEG = Checkbutton(self.frame_import, text = "Plot spectrogram with markers",
                                  font = 'Calibri 11 ', variable = self.plot_tfr_EEG)
        
        self.checkbox_plot_tfr_EEG.grid(row = 3, column = 4)
    ...
    def Apply_button(self):
        ...
        if 'hypnodyne_files_list' not in globals():
            ...
            if int(self.plot_tfr_EEG.get()) == 1:
                ...
                fig, markers_details = self.AssignMarkersToRecordedData_EEG_TFR(data = self.sigScript_org, data_R = self.sigScript_org_R, sf = 256,\
                 ...
                 fig = self.AssignMarkersToRecordedData_EEG(data = self.sigScript_org, data_R = self.sigScript_org_R, sf = 256,\
                                         path_to_json_markers=self.path_to_json_markers,\
                                         markers_to_show = ['light', 'manual'])
